...
public class CalculadoraService
{
    public CalculadoraTelaVM Calcular(CalculadoraTelaVM vm)
    {
        ...
        // 1b. Fila "URDIMBRE REFUERZO": en el Excel (Hoja2) NO son celdas de
        // entrada independientes, son fórmulas derivadas de la fila URDIMBRE:
        //   B5 = +B3*2   (Urdimbre Refuerzo Tejido = 2x Urdimbre Tejido)
        //   D5 = +D3     (Denier Refuerzo = mismo Denier de Urdimbre)
        // Se recalculan siempre aquí para que la web se comporte igual que
        // el Excel, ignorando cualquier valor que llegue del formulario.
        vm.UrdimbreRefuerzoTejido = vm.UrdimbreTejido * 2m;
        vm.DenierRefuerzo = vm.UrdimbreDenier;

        // 1c. "MEDIDA" (Hoja2, celda B10): =+CONCATENATE(B7,"x",(100))
        // El "100" está fijo (literal) en la fórmula del Excel, no es una
        // celda de entrada. "Corte" se fija siempre en 100 por esa razón.
        vm.Corte = 100m;
        ...
        // Peso individual del Refuerzo (Excel Hoja2, celda E12): =(B5*D5)/228.6
        vm.PesoRefuerzo = (vm.UrdimbreRefuerzoTejido * vm.DenierRefuerzo) / 228.6m;
