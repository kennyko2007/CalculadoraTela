document.addEventListener("DOMContentLoaded", function () {
    ...
    // ── Feedback visual de guardado (toast + estado del botón) ──
    const toastGuardado = document.getElementById("toastGuardado");
    const toastGuardadoMsg = document.getElementById("toastGuardadoMsg");
    const btnGuardarIcon = document.getElementById("btnGuardarIcon");
    const btnGuardarLabel = document.getElementById("btnGuardarLabel");
    let toastTimer = null;

    function mostrarToast(mensaje, esError) {
        if (!toastGuardado) return;
        clearTimeout(toastTimer);
        toastGuardadoMsg.textContent = mensaje;
        toastGuardado.classList.toggle("toast-error", !!esError);
        toastGuardado.classList.add("show");
        toastTimer = setTimeout(() => toastGuardado.classList.remove("show"), 3200);
    }

    function ponerBotonEnEstado(estado) {
        if (!btnGuardar) return;
        if (estado === "guardando") {
            btnGuardar.disabled = true;
            if (btnGuardarIcon) btnGuardarIcon.outerHTML = '<span class="btn-guardar-spin" id="btnGuardarIcon"></span>';
            if (btnGuardarLabel) btnGuardarLabel.textContent = "GUARDANDO...";
        } else if (estado === "exito") {
            const icon = document.getElementById("btnGuardarIcon");
            if (icon) icon.outerHTML = '<i class="bi bi-check-circle-fill fs-5" style="color:#34d399;" id="btnGuardarIcon"></i>';
            if (btnGuardarLabel) btnGuardarLabel.textContent = "¡GUARDADO!";
        } else {
            btnGuardar.disabled = false;
            const icon = document.getElementById("btnGuardarIcon");
            if (icon) icon.outerHTML = '<i class="bi bi-bookmark-check-fill fs-5" style="color:#fbbf24;" id="btnGuardarIcon"></i>';
            if (btnGuardarLabel) btnGuardarLabel.textContent = "GUARDAR REGISTRO EN HISTORIAL";
        }
    }

    function enviarFormulario() {
        const formData = leerFormulario();
        ponerBotonEnEstado("guardando");
        ...
        .then(data => {
            if (data && data.success) {
                ponerBotonEnEstado("exito");
                mostrarToast("¡Registro guardado con éxito en el historial!", false);
                setTimeout(() => { window.location.href = '/Home/Historial'; }, 900);
            } else {
                ponerBotonEnEstado("normal");
                mostrarToast("Error al guardar el registro: " + (data?.message || "Verifica los datos."), true);
            }
        })
        .catch(error => {
            console.error("Error en la solicitud:", error);
            ponerBotonEnEstado("normal");
            mostrarToast("Ocurrió un error al intentar conectar con el servidor.", true);
        });
    }
});
