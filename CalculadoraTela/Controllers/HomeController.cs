...
namespace CalculadoraTela.Controllers
{
    public class HomeController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> BorrarCalculo(int id)
        {
            try
            {
                var calculo = await _context.Calculos.FindAsync(id);
            ...
            {
                _logger.LogError(ex, "Error al borrar el registro {Id} del historial.", id);
        ...
        public async Task<IActionResult> VaciarHistorial()
        {
            try
            {
                // ExecuteDeleteAsync borra todo directamente en la base de datos,
                // sin traer los registros a memoria: rápido incluso si la tabla creciera.
                var borrados = await _context.Calculos.ExecuteDeleteAsync();
                return Json(new { success = true, borrados });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al vaciar el historial.");
                return Json(new { success = false, message = "No se pudo vaciar el historial." });
            }
        }
